...
void WriteNandPage(mode2_msg_data_struct* pmsg)
{
  ...
  status = nand_8bit_write_page(1,&pmsg->data[0],page_adr);
...
{
  uint16_t NAZ;
  uint16_t byte_per_az;
  uint16_t page_per_az;
  uint16_t total_az;
...
uint32_t GetCrc(void)
{
  ...
  for(i = 0; i < crc_initial_info.total_az; i++)
  {
    ...
    {
       status = nand_8bit_read_page(1,(uint8_t*)&f_data.data[j][0],page_adr++);
